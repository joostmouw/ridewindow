...
- **No backend (revised in v3.0):** From v3.0 onward the app uses managed **Supabase Auth + Postgres** for signed-in accounts — local Drift/SharedPreferences remain the source of truth on-device, and the app stays fully functional signed-out. The server-side exception is **`plpgsql` functions only**, six of them today, each earning its place by doing something the client provably cannot: four are called via `rpc()` because they must be a single transaction or must read rows the caller may not select directly (`migrate_account_data` for first-login migration, `delete_own_account`, `friend_profiles`, `redeem_friend_invite`), one is a `security definer` helper that RLS policies call (`is_ride_member`), and one is a trigger function that stamps `updated_at` (`set_updated_at`). New functions are justified in the migration that adds them. No Edge Functions, no other server-side code.
