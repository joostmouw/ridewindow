# Requirements Archive: v2.0 iOS Web App
...
- [x] **WEB-01**: `web/` platform is added via `flutter create --platforms web .` and committed
- [x] **WEB-02**: `flutter build web --release` (CanvasKit renderer, not `--wasm`) builds without errors
- [x] **WEB-03**: Existing UI (Welcome, Onboarding, Home, Ride Detail, Profile, Availability) renders and navigates correctly via `go_router` when run in a browser, with zero domain/UI code changes
- [x] **WEB-04**: Native-only plugins with no web implementation (`workmanager`, `home_widget`) are guarded with `kIsWeb` checks at their call sites in `main.dart`
- [x] **WEB-05**: A `flutter build apk` regression check confirms the existing Android app still builds and runs unchanged after web-platform additions

### Persistence (PERS) — Web storage backend

- [x] **PERS-05**: Drift database uses `DriftWebOptions` (`sqlite3.wasm` + `drift_worker.dart.js`) on web, alongside the existing native `DriftNativeOptions`
- [x] **PERS-06**: Data written to Drift on web survives a full page reload (manually verified, not just assumed from a successful build)
- [x] **PERS-07**: `sqlite3.wasm` is served with `Content-Type: application/wasm` from Firebase Hosting (explicit `firebase.json` header rule if needed)

### Location (LOC) — Web geolocation + fallback

- [x] **LOC-06**: Geolocation works via the browser Geolocation API (`geolocator_web`) over HTTPS on the deployed Firebase Hosting domain
- [x] **LOC-07**: On web, geolocation permission denial or timeout immediately surfaces the manual city picker as the primary path (not a rare fallback), reflecting iOS Safari's per-session re-prompt behavior

### Refresh (REFRESH) — Foreground refresh strategy (WorkManager replacement)

- [x] **REFRESH-01**: A `kIsWeb`-gated foreground refresh trigger calls `ref.invalidate(weatherProvider)` on page load and on regaining focus/visibility
- [x] **REFRESH-02**: User can manually pull-to-refresh to force a re-fetch
- [x] **REFRESH-03**: UI displays a "Last updated HH:MM" label reflecting the cache-then-network pattern
- [x] **REFRESH-04**: When a fetch fails (offline or error), the UI shows the last-known slots clearly labeled as stale, rather than a blank screen or crash
...
- [x] **DEPLOY-01**: `firebase.json` configures the Flutter web build output (`build/web`) as the Hosting public directory with correct rewrite rules for client-side routing
- [x] **DEPLOY-02**: The app is deployed to Firebase Hosting via `firebase deploy` and reachable at a stable HTTPS URL
- [x] **DEPLOY-03**: A full regression pass confirms the existing Android app (build, run, core flows) is unaffected by the web-platform additions
...
|-------------|-------|--------|
| WEB-01 | Phase 11 | Complete |
| WEB-02 | Phase 11 | Complete |
| WEB-03 | Phase 11 | Complete |
| WEB-04 | Phase 11 | Complete |
| WEB-05 | Phase 11 | Complete |
| PERS-05 | Phase 12 | Complete |
| PERS-06 | Phase 12 | Complete |
| PERS-07 | Phase 12 | Complete |
| LOC-06 | Phase 13 | Complete |
| LOC-07 | Phase 13 | Complete |
| REFRESH-01 | Phase 14 | Complete |
| REFRESH-02 | Phase 14 | Complete |
| REFRESH-03 | Phase 14 | Complete |
| REFRESH-04 | Phase 14 | Complete |
