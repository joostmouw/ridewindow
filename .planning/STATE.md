...
|---|---|---|
| 2026-09-21 | [agenda-export in de taal van de app](quick/260921-p3d-agenda-export-in-de-taal-van-de-app-en-d/) | Agenda-titel/-omschrijving en de deel-tekst waren altijd Nederlands en km/u -- **`_shareSlot()` lekte dat een tweede keer**, los van de agenda-knop. `CalendarService` is nu een pure datalaag (title/description als parameter); `ride_detail_screen.dart` bouwt de tekst via `S.of(context)` en `unitsProvider`, gedeeld door beide knoppen. 7 nieuwe regressietests (NL/EN, mph, Bft, °F, lege forecast), suite 744/744, analyze op de baseline van 200. Twee dingen bewust buiten scope: de homescreen-widget zet nog vast Nederlands, en `'Aanmelden geannuleerd'` vertaalt niet symmetrisch met `availability_screen.dart`. |
| 2026-09-21 | licentie van de intro-opname | **De opname mag mee in een Play-upload.** De bron heette `freesound_community-...-105846` maar staat op **Pixabay**, niet op freesound: 105846 is een Pixabay-id, en daarom vond de vorige ronde daar een synthesizer-kick van iemand anders. Gevonden met de download-metadata die macOS zelf bewaart (`kMDItemWhereFroms`), niet met zoeken op de naam. Pixabay Content License: commercieel gebruik en bewerken toegestaan, naamsvermelding niet verplicht. Vastgelegd naast de asset in `assets/sounds/WELCOME_ROLL-LICENSE.txt`, zoals de fonts het ook doen. Ook de intro-animatie is thuisgebracht: met AI laten maken in opdracht van Joost, dus eigen werk (`assets/animations/WELCOME_RIDE-ORIGIN.txt`). **Elk asset in de app heeft nu een vastgelegde herkomst**, en de regel die eruit volgt staat in OPEN.md punt 13: leg de herkomst vast als een bestand binnenkomt, niet als een release erop wacht. |
| 2026-09-21 | [lokaal account: zonder Gmail](quick/260921-lokaal-account/) | OPEN.md punt 11: **"Inloggen met e-mail" als tweede optie in Profiel**, e-mail + wachtwoord via Supabase (elk geldig adres). Bevestigingsmail staat aan; de dialoog zegt het als die onderweg is. Na succes dezelfde afronding als Google-login. 13 nieuwe ARB-strings, 5 regressietests, suite 727/727. Toestelcontrole (aanmaak + bevestiging + inloggen met eigen adres) staat open. |
| 2026-09-21 | [peloton-systeem](quick/260921-gxb-peloton-systeem-uit-schets-014/) | Schets 014 gebouwd: **links op de Home-kaart staat nu wat voor rit het is** -- het peloton-lint bij een groepsrit, één fietser als je alleen gaat -- en de rolregel zegt wat jij erin bent. De vlag is weg, de megafoon zegt dat jij organiseert, meerijden is tekst zonder icoon. Nieuw is de teller: een fietsje per persoon met de wachtenden doorzichtig, bij élke gedeelde rit en niet alleen bij die van jezelf. Het lint is de échte Phosphor-fietser drie keer over elkaar, samengevoegd tot één pad in Dart -- geen icoonfont, want `Icon` dwingt een vierkant af. Op localhost bleek de tellerzin op Home af te kappen; de teller meet nu zelf de ruimte en kort de zin in plaats van hem af te kappen. 722 tests groen, uitgerold als **1.0.38+49** op internal, alpha en de PWA, en door Joost op het toestel goedgekeurd. |
...
**Fase 23 is af** — alle zes de stappen uit schets 001 staan, plus de
Peloton-kaarten. De volgende vraag is welke fase van v4.0 aan de beurt is:
...
1. **De radii vormen geen systeem.** 24 op ritkaarten, 24 in `cardTheme`, 18 op
...
| 260903-b44 | backlog-60-cloudsyncreconciler-injecteer | Backlog #60: `CloudSyncReconciler` greep in zijn eigen methodebody naar `Supabase.instance.client`, dus van buitenaf was niet waarneembaar wát hij deed -- de reden dat fase 21 vijf gap-closure-plannen nodig had die stuk voor stuk pas op een toestel gevonden werden (21-10 toetste dát de aanroep bestond, 21-11 of die zijn werk bereikte, 21-12 of de payload een legale rij was; elke test stopte een laag te vroeg). Nieuw `CloudSyncGateway`: zes leden, elk precies één bestaande aanroep, plus `SupabaseCloudSyncGateway` met de calls ongewijzigd. Bewust géén mock van `SupabaseClient` -- die heeft een fluent builder, en dan toets je de builder in plaats van je eigen gedrag; dit volgt het patroon dat `AccountSyncService` (closures) en `RideDetailScreen` (optionele factory met productie-default) al gebruiken. 21-11's eis is niet verdwenen maar verhuisd: de `Supabase.instance`-lookup is in de gateway een getter per aanroep, want bij constructie zou hij gooien vóór de code die je juist wilt observeren. Nieuwe test (8) legt vast wat alleen een toestelsessie zag: push-vóór-pull (21-14), twee drains per cyclus, dat de opstart-reconcile werkelijk léést, en 21-13's reparatie van niet-canonieke `ride_id`'s -- dat laatste pad staat in REGRESSION-CHECKLIST-21.md §5b als permanent open vinkje omdat het op een toestel niet meer uit te lokken is. De volgordetest is aantoonbaar load-bearing: de volgorde is tijdelijk omgedraaid en de test faalde, daarna teruggedraaid. Geen gedragswijziging. Suite **459/459**, analyze 0 errors (169 tegen baseline 170) | 2026-09-03 | 8c04df9 |
| 260903-bey | backlog-57-adoptie-van-een-cloud-rij-enq | Backlog #57: elke koude start stuurde profiel én beschikbaarheid opnieuw op zonder wijziging. **Het item had de oorzaak mis** — `enqueueCurrentState()` hangt uitsluitend aan `pushLocalToCloud` en draait alleen bij inloggen. Werkelijke oorzaak is een lus die zichzelf voedt: de reconcile neemt een nieuwere cloud-rij over met `save(stamp: false)`, `save()` enqueuede onvoorwaardelijk (de vlag dekte de tijdstempel wel en de enqueue niet), de drain duwt de rij terug, de trigger `profiles_set_updated_at` (`before update`) zet `updated_at = now()`, en de volgende start ziet de cloud opnieuw als nieuwer met het hele verstreken verschil. Dat raakt méér dan dataverkeer: `updated_at` is de invoer waarop `resolveAccountSync()` beslist wie wint bij een divergentie. `save()` heeft nu een eigen `enqueue`-vlag; vier adoptie-plekken geven `enqueue: false`. Bewust niet aan `stamp` gekoppeld — `setUserNameFromSignIn()` schrijft met `stamp: false` én moet de cloud wél halen; daar staat nu een test op die die samentrekking blokkeert. Verklaart ook de tegenstrijdigheid uit device session 10 (`0 sent` op een verse installatie tegenover `1 sent` in sessie 5): na de migratie lagen beide kanten binnen de noop-band, dus geen adoptie en dus geen lus. Meetbaar geworden dankzij #60; de test faalt aantoonbaar zonder de fix en toetst eerst dát er geadopteerd is. Suite **462/462**, analyze onveranderd 169 / 0 errors | 2026-09-03 | 1ac8f3f |
