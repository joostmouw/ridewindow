...
|---|---|---|
| 2026-09-21 | licentie van de intro-opname | **De opname mag mee in een Play-upload.** De bron heette `freesound_community-...-105846` maar staat op **Pixabay**, niet op freesound: 105846 is een Pixabay-id, en daarom vond de vorige ronde daar een synthesizer-kick van iemand anders. Gevonden met de download-metadata die macOS zelf bewaart (`kMDItemWhereFroms`), niet met zoeken op de naam. Pixabay Content License: commercieel gebruik en bewerken toegestaan, naamsvermelding niet verplicht. Vastgelegd naast de asset in `assets/sounds/WELCOME_ROLL-LICENSE.txt`, zoals de fonts het ook doen. Nieuw punt 13 in OPEN.md: de intro-animatie is nu het enige asset zonder vastgelegde herkomst. |
