...
**Key context:** Slice 7 ("clubs") van epic #65 Peloton v2, besloten door Joost op 2026-09-23 (ontwerp: https://claude.ai/artifact/VLK7ktJhi4a75SALjRMN7W). Loopt als GSD-workstream `clubs` naast v4.1, die op de veertien-dagenklok wacht; fase 28 schuift daarvoor op. Fasenummering gaat verder na 32. Groepen hebben slice 3 (gedeelde beschikbaarheid) niet nodig: ze bouwen op het stemmen uit 0010. Het beschikbaarheidsrooster uit Profiel blijft privé. Migratie 0012 voegt vijf plpgsql-functies toe (`redeem_group_invite`, `is_group_member`, `create_group`, `guard_group_member_insert`, `ensure_group_admin`) en brengt het totaal op elf, omdat grenzen en opvolging alleen met triggers in de database af te dwingen zijn en atomisch aanmaken een rpc vraagt; de "No backend"-constraint in `CLAUDE.md` en `AGENTS.md` wordt daarop bijgewerkt. Fase 34 (herziening Joost 2026-09-23: voordragen en goedkeuren) voegt met 0013 `propose_group_member` en `accept_group_request` toe; het totaal wordt dertien. Bekende valkuilen uit `.planning/PELOTON.md` gelden hier dubbel: policy-recursie (security definer-helper nodig) en `insert ... returning` dat op de select-policy stukloopt.
...
- **Budget:** ~€25 one-time (Google Play Developer account) + Firebase Hosting free tier for the web build, plus an explicit ongoing ceiling of **€0/month** from v3.0 onward now that Supabase Auth + Postgres are added. That ceiling buys two consequences: the Supabase free tier has **no automated backups** (a lost hosted database has no restore point — blast radius limited only because the device keeps its own copy), and a free project **pauses after 7 days without API traffic** (mitigated, not eliminated, by a keep-warm job). Exceeding €0/month is a conscious future decision.
- **Timeline:** Realistic 8–12 weeks side-project pace. Acceptable to ship a thin v1 fast and iterate.
- **No backend (revised in v3.0):** Drift remains local storage (native + web IndexedDB/wasm) and stays the source of truth on-device; the app is fully functional signed-out. From v3.0 onward, signed-in users additionally get managed **Supabase Auth + Postgres**, accepting the auth/GDPR tier that v1.0 and v2.0 deliberately avoided. No server-side code beyond one `plpgsql` function, invoked via `rpc()`, for transactional first-login migration.
- **Privacy (revised in v3.0):** Location permission is the only sensitive permission. Privacy policy required (Play Store mandate; also linked from the web app). A signed-in user's profile, availability and planned rides are stored server-side in the EU — Supabase, **West EU (Paris), `eu-west-3`** — while the forecast cache and calendar-imported blocks deliberately stay local. There are now **two** sub-processors: Supabase (auth, database) and Google (Firebase Hosting, Calendar). A signed-out user's data still never leaves the device — that remains true and is the whole point of accounts being additive. **Herzien in v4.1 voor gebruiksstatistiek:** de app mag anoniem bijhouden hoe hij gebruikt wordt, maar **alleen na expliciete toestemming** die bij de eerste start gevraagd wordt en in Profiel om te draaien is. Zonder een 'ja' verlaat er nog steeds niets het toestel — uitgelogd noch ingelogd. Er is geen derde sub-processor: de gebeurtenissen gaan naar dezelfde Supabase-instantie in Parijs. Een gebeurtenis draagt nooit een locatie, nooit vrije tekst en nooit een e-mailadres; wel een willekeurige toestel-id die de gebruiker met het uitzetten van de toestemming weggooit. The published policy must be legally rewritten to reflect this before the first release with accounts (PRE-05, separate plan).
