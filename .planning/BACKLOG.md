...
| 17 | **Los tikken/slepen op aaneensluitende uren moet samensmelten tot één tijdvak (Availability)** — *(verduidelijkt n.a.v. quick-260714-o54 test-feedback, was: "drag werkt niet")*. Root cause: `_onCellTap` (lib/features/availability/availability_screen.dart:517) toggelt elk uur onafhankelijk; alleen een ononderbroken sleep-gebaar (`_onDragStart/_onDragUpdate/_onDragEnd`) selecteert meerdere uren als één actie. In de praktijk tikken gebruikers vaak los (indrukken-loslaten per uur, geen ononderbroken sleep) en verwachten dat aangrenzende uren (zelfde dag, opeenvolgend) dan alsnog samensmelten tot één doorlopend tijdvak — via slepen én via losse tikken, beide moeten mergen | HOOG | M | Backlog |
...
| 34 | **"Plan ride"-knop sticky op Ride Detail** — *(gecorrigeerd, was verkeerd toegeschreven aan Availability — die knop bestaat daar niet)*. Op Ride Detail (lang scherm: Adjust Time → Weather → Hourly-tabel → Plan ride) moet de groene "Plan ride"-knop onderin vastgeplakt (sticky) blijven tijdens het scrollen/aanpassen, i.p.v. pas zichtbaar na volledig doorscrollen | MEDIUM | S | Backlog |
| 35 | **Live teller "X losse tijdvakken" tijdens selecteren op Availability** — tijdens het aanvinken/slepen in de wekelijkse beschikbaarheids-grid toont de app hoeveel losse (niet-aaneengesloten) tijdvakken je nu hebt geselecteerd. Geen claim over exact aantal ritten (dat bepaalt de score-engine later, elders) — puur een live selectie-indicator | MEDIUM | S | Backlog |
