...
- [ ] **PRE-08**: A billing alert is configured on whatever surface incurs spend, documented explicitly as a notification tripwire and not a spend cap; and a conscious decision is recorded about Supabase's free-tier pause (free projects pause after 7 days with no API requests) — accept it, keep the project warm, or budget for Pro — together with what to do when a tester reports that sync stopped working
