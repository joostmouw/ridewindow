...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test 5 (Rule 6): with a work-blocked cell seeded at (day 0, hour 6) and a planned ride seeded covering (day 0, hour 20), open an anchor on (day 0, hour 9) ("1 uur geselecteerd"), then tap the blocked cell (day 0, hour 6) -> no change (still "1 uur geselecteerd", checkmark still only on hour 9). Then tap the planned cell (day 0, hour 20) -> still no change.
    - Test 6 (Rule 7): with real forecast/score data seeded for every grid cell, long-press a free cell with no active selection -> the weather-detail bottom sheet opens (assert its hour-range text, e.g. "9:00 – 10:00"). Dismiss it, open an anchor on (day 0, hour 9), then long-press the work-blocked cell (day 0, hour 6) -> the sheet still opens, AND the active selection is untouched afterward ("1 uur geselecteerd" still visible once the sheet is dismissed). Long-pressing the currently-selected anchor cell itself also opens the sheet (tap and long-press coexist without one canceling the other).
  ...
  In `setUp`, call `SharedPreferences.setMockInitialValues({'hint_seen_agenda': true})` (suppresses the hint overlay per the interfaces note). At the top of `main()`, compute `final now = DateTime.now(); final today = DateTime(now.year, now.month, now.day);` once, plus a helper `DateTime cellTime(int n, int hour) { final day = today.add(Duration(days: n)); return DateTime(day.year, day.month, day.day, hour); }` — this two-step construction (add whole days first, then rebuild the `DateTime` at the target hour via field construction) mirrors production's exact cell-timestamp derivation in `week_agenda_screen.dart` (`final day = today.add(Duration(days: i)); ...; DateTime(day.year, day.month, day.day, hour)`) rather than combining `days` and `hours` into a single `Duration`, so results stay identical to production across a DST transition week. Use `cellTime(n, hour)` for every `blockedHours` map-key seed and every `PlannedRide.start`/`.end` seed (both need this exact local, non-UTC, day-then-hour shape per the interfaces note on `_isBlocked`/`_isPlanned` direct-match semantics); use plain `today.add(Duration(days: n))` only where a day-level boundary with no specific hour is needed.
