...
| DPA **executed** (PandaDoc signed) | **Yes** — signed 2026-07-26 05:45 UTC | ✓ verified in the PDF structure, see F-4 |
...
> **Reopened and closed again on 2026-09-21.** Correcting the constant was not enough. The row sat behind `if (await canLaunchUrl(uri))` with no `else`, and on Android 11+ `canLaunchUrl` returns false unless the manifest declares `<queries>` for `VIEW`/`https` — which neither the app nor `url_launcher_android` did. So the link still opened nothing, silently, and a tester who then tapped the Open-Meteo row below it reported that the privacy policy opened a "free weather api". The manifest now declares the intent, and the silent branch is gone: both rows go through one helper that reports failure and offers the address on the clipboard. The lesson for this table is that "the URL is correct" and "the link works" are two different claims, and only the second one is what a tester experiences.
...
**F-4 (resolved 2026-07-26). The DPA is executed.** Clicking *Request DPA* only starts the process — Supabase prepares a **PandaDoc** document and emails a signing link (stated lead time: up to 24 hours). In this case it arrived and was signed the same morning. Signed by Joost Mouw as a natural person; document version `Template Supabase Customer DPA (June 1)`.

*How this was verified, because the first check got it wrong:* extracting the PDF's text layer shows blank underscores where the signature block is, which is just the template's printed ruling — that reading produced a false "unsigned" conclusion. The reliable evidence is in the PDF structure: `/FT/Sig` with a non-empty `/V`, a `/Filter/Adobe.PPKLite` signature dictionary, and a `/ByteRange` entry, signing time `D:20260726054529Z`, `/Creator PandaDoc.com`. **Check `/ByteRange`, not the text layer**, when verifying whether a PDF is signed.
